...
namespace Jst.UtilStandard.UtilsHelper
{
    ...
    public static class EncryptHelper
    {
        ...
        {
            return salt.IsEmpty()? value.Md532() : (value + "『" + salt + "』").Md532();
        }
        ...
        public static string HmacSHA256(this string value, string keyVal)
        {
            ...
            HMACSHA256 HmacSha256 = new HMACSHA256(encoding.GetBytes(keyVal));
            return HashAlgorithmBase(HmacSha256, value, encoding);
        }
        ...
        public static string Des(this string value, string keyVal, string ivVal)
        {
            try
            {
               ...
               ICryptoTransform transform =  des.CreateDecryptor(Encoding.UTF8.GetBytes(keyVal.Length > 8 ? keyVal.Substring(0, 8) : keyVal), Encoding.UTF8.GetBytes(ivVal.Length > 8 ? ivVal.Substring(0, 8) : ivVal));
                byte[] result = transform.TransformFinalBlock(data, 0, data.Length);
                return BitConverter.ToString(result);
            }
            catch (Exception ex) { return "转换出错！"; }
        }
        ...
        {
            if (encode.IsNull()) encode = Encoding.UTF8;
            return encode.GetString(System.Convert.FromBase64String(ciphervalue));
        }
        #endregion
